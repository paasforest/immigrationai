...
import { ArrowRight, CheckCircle, AlertTriangle, FileText, Globe, Shield, Zap, MapPin, GraduationCap, Briefcase } from 'lucide-react';
...
export default function UKVisaEligibilityPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok && data.success) {
        setResult(data.data);
        trackEvent('uk_visa_eligibility_check', { visaType: formData.visaType });
      } else {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50">
        ...
        <div className="max-w-7xl mx-auto grid gap-8 lg:grid-cols-2">
          ...
          <div className="bg-white rounded-3xl shadow-xl border border-slate-100 p-8">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <label className="text-sm font-medium text-slate-700 mb-2 block">UK Visa Type</label>
                <select
              ...
              <div>
                <label className="text-sm font-medium text-slate-700 mb-2 block">English Language Test</label>
                <select
                  className="w-full rounded-xl border-slate-200"
              ...
              >
                {loading ? 'Checking...' : (
                  <>
                    <span>Check UK Visa Eligibility</span>
          ...
          <div className="bg-slate-900 text-white rounded-3xl shadow-xl p-8">
            {result ? (
              ...
              <div>
                <h3 className="text-2xl font-bold mb-4">Why Check UK Visa Eligibility?</h3>
                <ul className="space-y-4 text-blue-100">
                  <li className="flex items-start space-x-3">
                    <CheckCircle className="w-5 h-5 text-emerald-400 mt-1 flex-shrink-0" />
                    <span>AI-powered assessment based on actual UK visa requirements</span>
                  </li>
                  <li className="flex items-start space-x-3">
                    <CheckCircle className="w-5 h-5 text-emerald-400 mt-1 flex-shrink-0" />
                    <span>Get instant feedback on your eligibility before applying</span>
                  </li>
                  <li className="flex items-start space-x-3">
                    <CheckCircle className="w-5 h-5 text-emerald-400 mt-1 flex-shrink-0" />
                    <span>Receive personalized document checklist</span>
                  </li>
                  <li className="flex items-start space-x-3">
                    <CheckCircle className="w-5 h-5 text-emerald-400 mt-1 flex-shrink-0" />
                    <span>Save time and money by knowing requirements upfront</span>
                  </li>
                </ul>
                <div className="mt-8 bg-white/10 rounded-2xl p-4 border border-white/20">
                  <p className="text-sm text-blue-200">
                    <strong>Popular UK Visa Routes:</strong> Skilled Worker Visa, Health & Care Worker Visa, Student Visa, Partner Visa, and more.
                  </p>
                </div>
              </div>
            )}
      ...
      <section className="py-12 px-4 sm:px-6 lg:px-8 bg-white">
        <div className="max-w-4xl mx-auto text-center">
          <h2 className="text-3xl font-bold text-gray-900 mb-4">Ready to Start Your UK Visa Application?</h2>
          <p className="text-xl text-gray-600 mb-8">Get AI-powered document generation, SOP writing, and interview prep.</p>
